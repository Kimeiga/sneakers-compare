...
const PriceTable = (props) => {
    ...
    let resellPrices = props.sneaker.resellPrices;

    for (let size in resellPrices?.stockX) {
        sizes.add(size);
    }
    for (let size in resellPrices?.flightClub) {
        sizes.add(size);
    }
    for (let size in resellPrices?.goat) {
        sizes.add(size);
    }
    for (let size in resellPrices?.stadiumGoods) {
        sizes.add(size);
    }
    ...
    console.log(sneaker)
    let isMinPrice = (price, size) => {
        let prices = [resellPrices?.stockX && resellPrices?.stockX[size], resellPrices?.flightClub && resellPrices?.flightClub[size], resellPrices?.goat && resellPrices?.goat[size], resellPrices?.stadiumGoods && resellPrices?.stadiumGoods[size]]
    ...
    return (
        <div class='table-card scroll-bar'>
            <Table responsive>
                ...
                <tbody>
                    <tr>
                        <td onClick={() => { window.open(sneaker.resellLinks?.stockX) }}><img src={stockXLogo} alt="stockXLogo"></img></td>
                        {Array.from(shoeSizes).map((size, index) => (
                            ...
                            <td key={index}> {resellPrices?.stockX && resellPrices?.stockX[size] ? <a
                                ...
                                href={sneaker.resellLinks?.stockX} target="_blank" rel="noopener noreferrer">${resellPrices.stockX[size]} </a> : '--'}</td>
                        ))}
                    </tr>
                    <tr>
                        <td onClick={() => {
                            sneaker.resellLinks?.flightClub &&
                                window.open(sneaker.resellLinks?.flightClub)
                        }}><img src={flightClubLogo} alt="flightClubLogo"></img></td>
                        {Array.from(shoeSizes).map((size, index) => (
                    ...
                    <tr>
                        <td class='goat-row' onClick={() => {
                            sneaker.resellLinks?.goat &&
                                window.open(sneaker.resellLinks?.goat)
                        }}><img src={goatLogo} alt="goatLogo"></img></td>
                        {Array.from(shoeSizes).map((size, index) => (
                    ...
                    <tr>
                        <td onClick={() => {
                            sneaker.resellLinks?.stadiumGoods &&
                                window.open(sneaker.resellLinks?.stadiumGoods)
                        }}><img src={stadiumGoodsLogo} alt="stadiumGoodsLogo"></img></td>
                        {Array.from(shoeSizes).map((size, index) => (
                            <td key={index}> {resellPrices?.stadiumGoods && resellPrices?.stadiumGoods[size] ? <a
                                ...
                                target="_blank" rel="noopener noreferrer">${resellPrices.stadiumGoods[size]} </a> : '--'}</td>
                        ))}
                    </tr>
                </tbody>
            </Table>
